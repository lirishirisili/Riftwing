...
func _check_resources() -> int:
	...
	if theme_src.find("ButtonPrimary") < 0 or theme_src.find("ButtonSecondary") < 0 or theme_src.find("ButtonTertiary") < 0:
		...
		return 1
	if scene_src.find("ButtonPrimary") < 0 or scene_src.find("ButtonSecondary") < 0 or scene_src.find("ButtonTertiary") < 0:
		printerr("main menu buttons not using theme variations")
		return 1
	for path in [
	...
	if scene_src.find("logo_riftwing") < 0 or scene_src.find("hero_vanguard_menu") < 0:
		...
		return 1
	if scene_src.find("cta_start_hex") < 0 or scene_src.find("ENDLESS MODE") < 0:
		printerr("main menu missing START RUN plate / endless copy")
		return 1
	print("resources_ok")
	return 0
...
func _check_runtime() -> int:
	...
	for size in [Vector2i(1080, 1920), Vector2i(1080, 2400), Vector2i(1080, 2478)]:
		...
		var start: Button = menu.get_node_or_null("%StartButton") as Button
		var daily: Button = menu.get_node_or_null("%DailyButton") as Button
		var ships: Button = menu.get_node_or_null("%ShipsButton") as Button
		var upgrades: Button = menu.get_node_or_null("%UpgradesButton") as Button
		...
		if safe_m != null and safe_m.get_theme_constant("margin_bottom") > 160:
			...
			fails += 1
		print("size_ok %dx%d title=%s start_h=%.0f ships_bottom=%.0f" % [
			size.x, size.y, title.text, start.custom_minimum_size.y, ships_bottom])
